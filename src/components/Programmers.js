...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    const index = listOfAwesome.findIndex(x => x.id === picked);
    return listOfAwesome[index].name;
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: picked === null ? 'royalblue' : '#FFD700', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          listOfAwesome.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setPicked(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
